...
async function proxy(req: NextRequest, ctx: RouteCtx) {
  ...
  if (!slug?.length) {
    return NextResponse.json({ message: "Missing auth path" }, { status: 400 });
  }

  const path = slug.join("/");
  const url = `${expressBackendOrigin()}/api/auth/${path}${req.nextUrl.search}`;
    ...
    return NextResponse.json(
      {
        ...
        detail,
        hint: "Start the API on port 4000: `cd backend && npm run dev`, or set `BACKEND_URL`.",
      },
      { status: 502 }
    );
  }
